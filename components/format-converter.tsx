...
export function FormatConverter({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (!inputFormats.includes(file.type)) {
        setError(`Please select valid ${inputLabel} files only`)
        continue
      }
  ...
  const acceptString = inputFormats.join(",")
    ...
    <div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
      ...
      {images.length === 0 ? (
        ...
        >
          ...
          <h3 className="mt-4 text-lg font-semibold text-foreground">Upload {inputLabel} Images</h3>
          ...
          {convertedImages.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              <Button className="w-full" size="lg" onClick={downloadAll}>
                ...
                Download All ({convertedImages.length} files)
              </Button>
            </div>
          )}
        </div>
