...
import { Upload, FileImage, Trash2, Download, AlertCircle, X } from "lucide-react"
...
const ALL_TYPES = ["image/jpeg", "image/jpg", "image/png", "image/webp"]
const MAX_FILE_SIZE = 10 * 1024 * 1024 // 10MB

export function ImageConverter({ acceptedFormats, formatLabel = "JPG, PNG, or WEBP" }: ImageConverterProps) {
  const [images, setImages] = useState<ImageFile[]>([])
  const [error, setError] = useState<string | null>(null)
  const [isConverting, setIsConverting] = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)

  const acceptedTypes = acceptedFormats || ALL_TYPES
  ...
  const validateFile = (file: File): string | null => {
    if (!acceptedTypes.includes(file.type)) {
      return `"${file.name}" is not a supported format. Please use ${formatLabel}.`
    }
  ...
  const acceptString = acceptedFormats
    ? acceptedFormats.map((t) => `.${t.split("/")[1]}`).join(",")
    : ".jpg,.jpeg,.png,.webp"
