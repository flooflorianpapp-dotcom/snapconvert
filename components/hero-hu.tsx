...
export function HeroHu() {
  ...
  const validateFile = (file: File): string | null => {
    if (!ACCEPTED_TYPES.includes(file.type)) {
      return `"${file.name}" nem támogatott formátum. Kérjük, használjon JPG, PNG vagy WEBP formátumot.`
    }
    if (file.size > MAX_FILE_SIZE) {
      return `"${file.name}" meghaladja a 10MB-os korlátot.`
    }
    return null
  }
  ...
  const convertToPDF = async () => {
    if (images.length === 0) {
      setError("Kérjük, töltsön fel legalább egy képet a konvertálás előtt.")
      return
    }
    ...
    try {
      ...
      pdf.save("snapconvert-kepek.pdf")
    } catch {
      setError("Nem sikerült a képeket PDF-be konvertálni. Kérjük, próbálja újra.")
    } finally {
